...
describe('SOverlay', () => {
  ...
  const props = reactive<Mutable<Partial<PropsOverlay>>>(defaultProps)
  ...
  it('display overlay/scrim if position is fixed', async () => {
    await overlayWrapper.setProps({ value: true, disabled: true, position: 'fixed' })
